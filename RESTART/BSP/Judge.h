...
#define BSP_UART5_DMA_TX_BUF_LEN        119     //裁判系统学生通讯字节最多为119，数据内容最多为113
...
typedef enum
{
    ...
    DART_SHOOT_STATE_ID                = 0x0004, //飞镖发射状态:0x0004.飞镖发射后发送
    ...
    DART_REMAINING_TIME_ID             = 0x0105, //飞镖发射倒计时：cmd_id (0x0105)。，1Hz周期发送
...
typedef __packed struct    //飞镖发射倒计时：cmd_id (0x0105)。，1Hz周期发送
{
    uint8_t dart_remaining_time;
} ext_dart_remaining_time_t;

typedef __packed struct    //飞镖发射状态:0x0004.飞镖发射后发送
{
    uint8_t dart_belong;
...
typedef __packed struct//子弹剩余发射数：0x0208。发送频率：1Hz 周期发送，空中机器人以及哨兵机器人主控发送
{
...
typedef struct
{
    ...
    ext_dart_remaining_time_t             dart_shoot_remaining_time;
...
extern receive_judge_t judge_rece_mesg;
extern uint8_t have_heat0_flag ;
