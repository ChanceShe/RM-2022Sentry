...
uint32_t receive_buf1 = 0;
uint32_t receive_buf2;
...
uint8_t  tx_buf[BSP_UART5_DMA_TX_BUF_LEN];       //裁判系统学生通讯发送数据
#endif
...
{
    DMA_Cmd ( DMA_Streamx, DISABLE );                   //关闭DMA通道传输
    while ( DMA_GetCmdStatus ( DMA_Streamx ) != DISABLE ) {}	//判断别的数据流是否在操作，确保DMA可以被设置
    DMA_SetCurrDataCounter ( DMA_Streamx, ndtr );      //设置数据传输量
    DMA_Cmd ( DMA_Streamx, ENABLE );                   //开启DMA通道传输
}

/**
  * @brief  裁判系统空闲中断(UART5)
  * @param  void
  * @retval void
...
//获取裁判系统信息,数据倒着读
uint8_t D_lenth;
void judgement_data_handle ( uint8_t *p_frame, u16 rec_len ) //串口5中断中执行
{
    ...
    while ( rec_len > deal_cnt )
    {
        ...
        data_length = ( ( u16 ) p_frame[deal_cnt + 2] << 8 ) | p_frame[deal_cnt + 1]; //数据帧中data的长度
        cmd_id      = ( ( u16 ) p_frame[deal_cnt + 6] << 8 ) | p_frame[deal_cnt + 5]; //数据帧的ID号
        data_addr   =  &p_frame[deal_cnt] + HEADER_LEN + CMD_LEN;     //数据帧中data的地址

        memcpy ( header, &p_frame[deal_cnt], HEADER_LEN );
        Frame_length = HEADER_LEN + CMD_LEN + data_length + CRC_LEN;

        if ( sof == DN_REG_ID && Verify_CRC8_Check_Sum ( header, HEADER_LEN )
...
//                    if ( data_length == 3 )
                ...
                case DART_SHOOT_STATE_ID ://飞镖发射状态数据：0x0004。发送频率：1Hz
                    if ( data_length == 3 )
                        memcpy ( &judge_rece_mesg.dart_status, data_addr, data_length );
                    break;

                case EVENT_DADA_ID://场地事件数据：0x0101。发送频率：事件改变后发送
                    if ( data_length == 4 )
                        memcpy ( &judge_rece_mesg.event_data, data_addr, data_length );   //前哨站，能量机关，补血点，增益点激活状态
                    break;
                ...
                case  DART_REMAINING_TIME_ID ://飞镖发射倒计时：飞镖发射15s倒计时。cmd_id (0x0105)。1Hz周期发送
                    if ( data_length == 2 )
                        memcpy ( &judge_rece_mesg.dart_shoot_remaining_time, data_addr, data_length );
                    break;
...
//                        heat0_limit();
                ...
                case BUFF_MUSK_ID://机器人增益：0x0204。发送频率：状态改变后发送
                    if ( data_length == 1 )
                        memcpy ( &judge_rece_mesg.buff_musk.power_rune_buff, data_addr, data_length );
                    break;
                ...
                case BULLET_REMAINING_ID://子弹剩余发送数，空中机器人以及哨兵机器人发送：0x0208。1Hz 周期发送
                    if ( data_length == 6 )
                        memcpy ( &judge_rece_mesg.bullet_remaining, data_addr, data_length ); //该信息包含剩余金币数目，分区赛不获取步兵和英雄的弹丸数目
                    break;

                case STUDENT_INTERACTIVE_HEADER_DATA_ID://交互数据接收信息，发送频率：上限 10Hz
                    memcpy ( &judge_rece_mesg.student_interactive_header_data, data_addr, 6 );
                    if ( judge_rece_mesg.student_interactive_header_data.data_cmd_id == 0x201 ) //哨兵信息
                    {
                        ...
                        if ( ( judge_rece_mesg.game_robot_state.robot_id == judge_rece_mesg.student_interactive_header_data.receiver_ID )
                                && ( judge_rece_mesg.student_interactive_header_data.send_ID == 6
                                     ||  judge_rece_mesg.student_interactive_header_data.send_ID == 106 ) )    //发送端飞机
                        {
                            VehicleReserveInfo.chassis_state     = judge_rece_mesg.robot_interactive_data.data[0];
                            VehicleReserveInfo.lower_shoot_state = judge_rece_mesg.robot_interactive_data.data[1];
                            VehicleReserveInfo.upper_shoot_state = judge_rece_mesg.robot_interactive_data.data[2];
//                            VehicleReserveInfo.lower_gimbal_state = judge_rece_mesg.robot_interactive_data.data[3];
                            VehicleReserveInfo.return_flag       = judge_rece_mesg.robot_interactive_data.data[4];
                        }
                    }
                    break;
...
{
    uint16_t frame_length = HEADER_LEN + CMD_LEN + len + CRC_LEN;
    protocol_packet_pack ( cmd_id, p_data, len, sof, tx_buf );     //串口发送协议，函数返回的tx_buf是要发送的数据
...
//  }
    ...
    if ( sof == DN_REG_ID )
    {
        while ( 1 )
        {
            if ( DMA_GetFlagStatus ( DMA1_Stream7, DMA_FLAG_TCIF7 ) != RESET ) //等待DMA1_Steam7传输完成
            {
        ...
        MYDMA_Enable ( DMA1_Stream7, frame_length );  //开启一次DMA传输
    }
}
