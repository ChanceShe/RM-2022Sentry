...
//    gim.pid.yaw_speed_fdb = GMYawGyro;     //  ½ÇËÙ¶È
...
//    gim.pid.pit_angle_fdb = GMPitchEncoder.ecd_angle;
    ...
    if ( (gim.pid.yaw_angle_fdb-gim.pid.yaw_angle_ref>=-2.0f) && (gim.pid.yaw_angle_fdb-gim.pid.yaw_angle_ref<=2.0f) && \
			 (gim.pid.pit_angle_fdb-gim.pid.pit_angle_ref >= -2.0f) && (gim.pid.pit_angle_fdb-gim.pid.pit_angle_ref<=2.0f) )    //ÔÆÌ¨»Øµ½³õÊ¼½Ç¶Èºó½øÈëÒ£¿ØÆ÷Ä£Ê½
		{
...
void gimbal_remote_handle(void)
{
    ...
    gim.pid.pit_angle_fdb = pitch_Angle;	//    ÏòÉÏÎªÕý
		gim.pid.yaw_angle_fdb = yaw_Angle;		//
//    gim.pid.pit_angle_fdb = GMPitchEncoder.ecd_angle;	//    ÏòÉÏÎªÕý
//		gim.pid.yaw_angle_fdb = GMYawEncoder.ecd_angle;		//
}
...
//						Gimbal_Auto_Shoot.target_pit = -(new_location.pitch - PITCH_ZERO);
    ...
    else
    {
        ...
        if ( Gimbal_Auto_Shoot.Recognized_Timer == 50 ) //200ms£¬Ê±¼äÌ«³¤£¬ÔÆÌ¨±£³ÖÔ­ÓÐ¸ø¶¨µ¼ÖÂÂÒ¶¯×÷
        {
...
//            if ( refromData.s1 != 3  &&  ( new_location.recogflag != 0 ) && ( refromData.JudgeShootFlag == 1 )) //´Ó¹Ø±Õµ½start turning
