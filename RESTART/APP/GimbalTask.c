...
void gimbal_task(void)
{
	 ...
	 switch (gim.ctrl_mode)
   {
    ...
    break;
		...
		case GIMBAL_NO_ARTI_INPUT:		//²ÎÊı³õÊ¼»¯
			no_action_handle();
		break;
...
//		PID_struct_init(&pid_yaw_speed, POSITION_PID,25000, 2000,175.0f,0.14f,30.0f);
		...
		PID_struct_init ( &pid_yaw, POSITION_PID , 200, 20,
                      10, 0.02, 30 );
		PID_struct_init ( &pid_yaw_speed, POSITION_PID , 28000, 5000,
                      160.0f, 0.001, 150 );
	//Ğ±ÆÂ³õÊ¼»¯
...
void cascade_pid_ctrl ( void )	//¼¶Áªpidº¯Êı
{
...
{
    auto_mode = AUTO_PATROL ; //Ñ²Âß£¬Ã»ÓĞÊ¶±ğµ½Ä¿±ê
...
//    gim.pid.pit_angle_ref = 0 * ( 1 - GMPitchRamp.Calc ( &GMPitchRamp ) ); //GMPitchEncoder.ecd_angle * (1 - GMPitchRamp.Calc(&GMPitchRamp));
  ...
  //s												((GMPitchEncoder.ecd_angle - Init_Pitch_Angle)/fabs(GMPitchEncoder.ecd_angle - Init_Pitch_Angle))	* (1 - GMPitchRamp.Calc(&GMPitchRamp));
    ...
    if ( (gim.pid.yaw_angle_fdb-gim.pid.yaw_angle_ref>=-1.5f) && (gim.pid.yaw_angle_fdb-gim.pid.yaw_angle_ref<=1.5f) && \
        ...
        gim.pid.yaw_angle_fdb = GMYawEncoder.ecd_angle;
//        GimbalRef.yaw_angle_dynamic_ref = yaw_Angle;		//ÍÓÂİÒÇÏòÓÒÎªÕı
			  GimbalRef.yaw_angle_dynamic_ref = GMYawEncoder.ecd_angle;		//ÍÓÂİÒÇÏòÓÒÎªÕı
        GimbalRef.pitch_angle_dynamic_ref = pitch_Angle;	//ÍÓÂİÒÇÏòÉÏÎªÕı
    }
}
...
{
		VAL_LIMIT ( GimbalRef.pitch_angle_dynamic_ref, PITCH_MIN, PITCH_MAX ); //pitchÖáÔÆÌ¨¸©ÑöÏŞÖÆ
	  gim.pid.yaw_angle_ref = GimbalRef.yaw_angle_dynamic_ref;
    gim.pid.pit_angle_ref = GimbalRef.pitch_angle_dynamic_ref;
    gim.pid.pit_angle_fdb = pitch_Angle;	//GMPitchEncoder.ecd_angle;    //ÏòÉÏÎªÕı
...
void gimbal_auto_handle(void)
{
	switch ( auto_mode )
	{
		case AUTO_PATROL:
    {
			gimbal_patrol_handle();      //1.  Ã»ÓĞÊ¶±ğµ½Ä¿±ê  ÔÆÌ¨Ñ²Âß
    }
    break;
    case AUTO_FOLLOW:
    {
      gimbal_follow_handle();      //2.  Ê¶±ğµ½Ä¿±ê ¸ú×ÙÄ¿±ê
    }
    break;
    default:
    break;
...
{
	  gim.pid.yaw_angle_fdb =  GMYawEncoder.ecd_angle;    //ÓÉ±àÂëÆ÷¶Áµ½
//    gim.pid.pit_angle_fdb =  GMPitchEncoder.ecd_angle;  
    gim.pid.pit_angle_fdb =  pitch_Angle;  
		if ( new_location .recogflag != 0 )                //Ò»µ©¼ì²âµ½Ä¿±ê¾ÍÍ£ÏÂ
    {
...
//        {
//            auto_lost_timer ++ ;
//            if ( auto_lost_timer >= 30 )
//            {
//                auto_lost = 0;
//                auto_lost_timer = 0;
//                pitch_timer = ( int16_t ) ( pitch_Angle / ( ( PITCH_MAX - PITCH_MIN ) / PITCH_PERIOD ) );
//                yaw_timer = ( int16_t ) ( ( yaw_Angle - Init_Yaw_Angle ) / ( 160 / YAW_PERIOD ) );
//            }
//            gim.pid.pit_angle_ref = pitch_Angle;
...
//					gim.pid.yaw_angle_ref = GMYawEncoder.ecd_angle + 3;
        if ( auto_lost )
        {
            auto_lost_timer ++ ;
            if ( auto_lost_timer >= 10 )         // if(auto_lost_timer >= 1000)
            {
                auto_lost = 0;
                auto_lost_timer = 0;
                pitch_timer = ( int16_t ) ( pitch_Angle / ( ( PITCH_MAX - PITCH_MIN ) / PITCH_PERIOD ) );
...
void gimbal_follow_handle(void)		//Ê¶±ğµ½Ä¿±ê¸úËæÄ£Ê½
{
    ...
    else
    {
        ...
        if ( Gimbal_Auto_Shoot.Recognized_Timer == 2 ) //200ms£¬Ê±¼äÌ«³¤£¬ÔÆÌ¨±£³ÖÔ­ÓĞ¸ø¶¨µ¼ÖÂÂÒ¶¯×÷
        {
...
//#if ENABLE_KALMAN_FILTER == 1	           //Ê¹ÓÃ¿¨¶ûÂüÂË²¨Ö®ºóµÄËÙ¶È½øĞĞ²¹³¥£¬²¹³¥Í¼Ïñ´¦ÀíÑÓ³ÙºÍµ¯µÀÑÓ³Ù
...
//        //1¡¢¸ÕÊ¶±ğµ½£¬Æ«²î½Ï´ó£¬Èç¹ûÖ±½Ó²¹³¥»áµ¼ÖÂÔÆÌ¨¶¶¶¯Õğµ´£»
//        //2¡¢Ê¶±ğµ½Ò»¶¨Ê±¼ä£¬µ«ÊÇÓÉÓÚÎŞ²¹³¥µ¼ÖÂ¸ú×ÙÖÍºó
//        //3¡¢ÔÆÌ¨ÔÚ¶¶¶¯£¬µ¼ÖÂyaw·´À¡ÔÚÖĞ¼äÌøÔ¾
//        else
//        {
//            Gimbal_Auto_Shoot.Continue_Large_Err_Cnt++;
//            if ( Gimbal_Auto_Shoot.Continue_Large_Err_Cnt >= 40 )  //300ms£¬³ÖĞø300msµÄÆ«²î£¬±íÃ÷´ËÊ±Îª¸ú×ÙÖÍºó£¬Ğè¼ÓÈë²¹³¥
//            {
...
//          //¸ù¾İÆ«²î½Ç¼ÆËãÆ«²î¾àÀë£¬ÓÉÓÚ¾àÀëĞÅÏ¢´æÔÚÎó²î£¬²»Ê¹ÓÃ¼ÆËãµÄtemp = fabs(Gimbal_Auto_Shoot.Distance * 2.0f * arm_sin_f32(0.5f * fabs(Gimbal_Auto_Shoot.Delta_Dect_Angle_Yaw) * 0.01745329252f ));
//          //Æ«²î½Ç¶ÈĞ¡ÓÚ2¶È£¬ËµÃ÷´ËÊ±»ù±¾Ãé×¼×°¼×Æ¬£¬Á½¸öÊ±¼ä²ÎÊıÓ¦¸ÃĞèÒªµ÷Õû¡£
//          //¸Ã´¦Ó¦¸Ã¼ÓÉÏpitchÖáµÄÔ¤²âÅĞ¶¨
//        if ( fabs ( Gimbal_Auto_Shoot.Delta_Dect_Angle_Yaw ) < 3.0 + fabs ( Gimbal_Auto_Shoot.Yaw_Gimbal_Delay_Compensation ) )
//        {
//            Gimbal_Auto_Shoot.Continue_Large_Err_Cnt = 0;
//            gim.pid.yaw_angle_ref += ( Gimbal_Auto_Shoot.Yaw_Gimbal_Delay_Compensation );
...
//        {
//            Gimbal_Auto_Shoot.Continue_Large_Err_Cnt++;
//            if ( Gimbal_Auto_Shoot.Continue_Large_Err_Cnt >=100 )  //300ms£¬³ÖĞø300msµÄÆ«²î£¬±íÃ÷´ËÊ±Îª¸ú×ÙÖÍºó£¬Ğè¼ÓÈë²¹³¥
//            {
...
//        gim.pid.yaw_angle_fdb =  yaw_Angle;//-GMYawEncoder.ecd_angle;//×îÖÕ×ª»»³É½Ç¶È  ÏµÊıĞèÒªµ÷Õû
//        gim.pid.pit_angle_fdb =  pitch_Angle;//GMPitchEncoder.ecd_angle;//×îÖÕ×ª»»³É½Ç¶È  ÏµÊıĞèÒªµ÷Õû
...
void auto_shoot_task(void)
{
    ...
    /*×Ô¶¯´ò»÷·¢Éä¿ØÖÆÂß¼­
    	1.ÓÉÓÚÎŞ·¨×°ÏŞÎ»¿ª¹Ø£¬ĞèÏÈ´ò¿ªÄ¦²ÁÂÖÔÙ´ò¿ª²¦ÅÌ£¬²»¿É±ÜÃâ»áÔì³ÉÒ»¶¨³Ì¶ÈÉÏµÄÑÓÊ±£¬³õ²½Îª100ms
    	2.¹Ø±Õ·¢ÉäÊ±Ó¦Í¬Ê±¹Ø±Õ²¦ÅÌºÍÄ¦²ÁÂÖ
    	3.×°¼×½øÈëÒ»¶¨ÇøÓòÊ±´¥·¢·¢Éä×´Ì¬AAA
    	4.ĞèÒªºÍÒ£¿ØÆ÷×´Ì¬ÎŞ·ìÏÎ½Ó±ãÓÚ¿ØÖÆ
    	5.Ğè²¦ÖÁÌØ¶¨µµÎ»²Å¿ªÆôÉä»÷¹¦ÄÜ
      6.¹Ø±ÕÖ®ºóÉÏµ¯ÖÁ¶Â×ª
