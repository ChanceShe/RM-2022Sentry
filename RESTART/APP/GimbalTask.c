...
//        GimbalRef.yaw_angle_dynamic_ref = yaw_Angle;		//ÍÓÂİÒÇÏòÓÒÎªÕı
        ...
        GimbalRef.pitch_angle_dynamic_ref = GMPitchEncoder.ecd_angle;	//ÍÓÂİÒÇÏòÉÏÎªÕı
    }
}
...
void gimbal_remote_handle(void)
{
    ...
    gim.pid.pit_angle_fdb = GMPitchEncoder.ecd_angle;	//GMPitchEncoder.ecd_angle;    //ÏòÉÏÎªÕı
...
{
	  gim.pid.yaw_angle_fdb =  GMYawEncoder.ecd_angle;    //ÓÉÍÓÂİÒÇ¶Áµ½
    gim.pid.pit_angle_fdb =  GMPitchEncoder.ecd_angle;  
...
void gimbal_follow_handle(void)		//Ê¶±ğµ½Ä¿±ê¸úËæÄ£Ê½
{
	    if ( new_location.id )
    {
        ...
        Gimbal_Auto_Shoot.Distance = new_location.dis*10;		//Ğè½«µ¥Î»cm×ª»¯Îªmm
    }
    else
    {
        Gimbal_Auto_Shoot.Recognized_Timer++;
        if ( Gimbal_Auto_Shoot.Recognized_Timer == 20 ) //200ms£¬Ê±¼äÌ«³¤£¬ÔÆÌ¨±£³ÖÔ­ÓĞ¸ø¶¨µ¼ÖÂÂÒ¶¯×÷
        {
