...
void shoot_friction_handle ( void ) //摩擦轮 根据   friction_rotor标志  来设置输出
{
    ...
    CAN2_Send_Msg ( CAN2 , pid_rotate[0].out , pid_rotate[1].out , 0 , 0 );

}
static void shoot_bullet_handle ( void ) //   根据 (GetShootState() == ????)  来设定给定
{
    int16_t frictioSpeed = 1880;
...
//        if ( !is_judge_on )
//            pid_trigger_speed.set = PID_SHOOT_NOJUDGE_MOTOR_SPEED;
//				else
//        {
...
//        }
        ...
        if ( start_shooting_count > 120 && abs ( PokeEncoder.filter_rate ) < 6  ) //开始了一段时间并且转速低于一定的值  说明堵转
        {
            lock_rotor = 1;
    ...
    {
        //if((GetShootState() == NOSHOOTING)||overheat ==1||shot.ctrl_mode==0)
